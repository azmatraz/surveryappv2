...
//	}
	...
	public SAppQuestions getAllQuestions() {
		...
		try {
			...
			rs = ps.executeQuery();
			if (rs.next()) {
				questions = new SAppQuestions();
				questions.setQuestionNo(rs.getInt("QuestionNo"));
				questions.setQuestionTitle(rs.getString("QuestionTitle"));
				questions.setQuestionBody(rs.getString("QuestionBody"));
				questions.setEmail(rs.getString("Email"));
				System.out.println("Question found::" + questions.getQuestionNo() +"question body" +questions.getQuestionBody());
			} else {
				System.out.println("No questions found found with id=" );
			}
		} catch (SQLException e) {
			e.printStackTrace();
		} finally {
